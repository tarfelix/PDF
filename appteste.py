...
Editor e Divisor de PDF Completo (PT-BR) – Versão com Proteção de Memória
-------------------------------------------------------------------------
• ... (todas as melhorias anteriores) ...
• NOVO: Proteção contra esgotamento de memória. A aba "Visual" é desabilitada
  para arquivos muito grandes para evitar que o aplicativo trave.
"""
...
# NOVO: Constante para o limite de tamanho da aba visual
...
def get_bookmark_ranges(doc: fitz.Document):
    ...
    for i, (lvl, title, page1, *_) in enumerate(toc):
        ...
        res.append({"id": f"bm_{i}_{page1}", "level": lvl, "title": title, "start_page_0_idx": start0, "end_page_0_idx": end0, "display_text": disp})
    return res
...
st.markdown("Carregue um ou mais PDFs e escolha a ação desejada. **Arquivos muito grandes (ex: > 50 MB) terão a aba 'Visual' desabilitada para evitar travamentos.**")
...
if uploaded_files:
    ...
    if file_ids != st.session_state.last_uploaded_file_ids:
        ...
        if len(uploaded_files) == 1:
            ...
            if file_size_mb > VISUAL_PREVIEW_SIZE_LIMIT_MB:
                visual_tab_enabled = False
                st.warning(f"⚠️ Arquivo grande ({file_size_mb:.1f} MB). A aba 'Visual' foi desabilitada para evitar consumo excessivo de memória. As outras funcionalidades continuam ativas.")
            else:
                visual_tab_enabled = True
    ...
    is_single_pdf_mode = True
    file_size_mb = len(st.session_state.pdf_doc_bytes_original) / (1024 * 1024)
    visual_tab_enabled = file_size_mb <= VISUAL_PREVIEW_SIZE_LIMIT_MB
...
if is_single_pdf_mode:
    tabs_to_show = ["Peças Jurídicas"]
    if visual_tab_enabled:
        tabs_to_show.append("Visual")
    tabs_to_show.extend(["Remover", "Extrair", "Dividir", "Otimizar", "Mesclar"])

active_tabs = st.tabs(tabs_to_show)
tab_map = {name: tab for name, tab in zip(tabs_to_show, active_tabs)}

# ===========================================================
# ABA MESCLAR (sem alterações)
# ===========================================================
with tab_map["Mesclar"]:
    st.header("🔗 Mesclar Múltiplos PDFs")
    files_to_merge = st.session_state.get('files_to_merge', [])
    if not files_to_merge and not is_single_pdf_mode: st.info("Para mesclar, carregue dois ou mais arquivos PDF no campo de upload acima.")
    elif not files_to_merge and is_single_pdf_mode: st.warning("Você carregou apenas um arquivo. Para mesclar, carregue múltiplos arquivos.")
    else:
...
if is_single_pdf_mode and doc_cached:
    # O código para as abas "Peças Jurídicas", "Remover", "Extrair", "Dividir", e "Otimizar"
    # permanece exatamente o mesmo da versão anterior, pois eles são eficientes com memória.
    # A única mudança é na aba "Visual".
                ...
                if not selected_pages: st.warning("Nenhuma peça foi selecionada para extração.")
                    ...
                    try:
                        with st.spinner("Gerando PDF com as peças selecionadas..."):
                            ...
                            generate_download_button(new_doc, output_filename, "Baixar Peças Extraídas", {"deflate_images": optimize_legal, "deflate_fonts": optimize_legal}, password_legal)
                    except Exception as e:
                        st.error(f"Erro ao extrair peças: {e}")

    # A aba Visual só será processada se estiver habilitada
    if visual_tab_enabled and "Visual" in tab_map:
        with tab_map["Visual"]:
                ...
                if len(sel_pages) >= doc_cached.page_count: st.error("Não é possível excluir todas as páginas do documento.")
                    ...
                    try:
                        with st.spinner("Excluindo páginas..."):
                            ...
                            generate_download_button(new_doc, output_filename, "Baixar PDF Modificado", password=password_visual)
                    except Exception as e:
            ...
            if col_ext.button("✨ Extrair Selecionadas", disabled=not sel_pages):
                try:
                    with st.spinner("Extraindo páginas..."):
                        ...
                        generate_download_button(new_doc, output_filename, "Baixar PDF Extraído", password=password_visual)
                except Exception as e:

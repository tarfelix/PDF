...
DEFAULT_STATE = {
    ...
    'found_legal_pieces': [], # Para armazenar as peças jurídicas encontradas
}
...
def get_bookmark_ranges(pdf_doc_instance):
    ...
    bookmarks_data = []
    if not pdf_doc_instance or not pdf_doc_instance.has_toc: return bookmarks_data
...
def find_legal_sections_by_bookmark(bookmarks_data):
    """Identifica peças jurídicas baseadas nos títulos dos marcadores."""
    found_pieces = []
            ...
            if not st.session_state.get('found_legal_pieces'):
                ...
                if st.button("Extrair Peças Selecionadas", key="process_legal_extract_button", disabled=st.session_state.get('processing_legal_extract', False)):
                    ...
                    if not selected_pages_to_extract:
                        ...
                        st.session_state.error_message = None
                        new_legal_doc = None
                        with st.spinner(f"Extraindo {len(selected_pages_to_extract)} página(s) das peças selecionadas..."):
                            try:
    ...
    if len(tabs) > tab_index_offset + 5:
        with tabs[tab_index_offset + 5]: # ABA: OTIMIZAR PDF
            ...
            optimization_profiles = {
                ...
                "Leve (Rápida, Sem Perda de Qualidade Visual)": "Remove dados desnecessários e aplica compressão básica.",
                "Recomendada (Equilíbrio entre Tamanho e Qualidade)": "Limpeza mais profunda e compressão de imagens/fontes.",
                "Máxima Tentativa (Pode Afetar Qualidade de Imagens)": "Usa todas as opções de compressão sem perdas."
            }
            selected_profile_name = st.selectbox(
                "Escolha um Perfil de Otimização:", options=list(optimization_profiles.keys()), index=2, 
                help="Selecione o nível de otimização desejado.", key="optimize_profile_select"
            )
            ...
            if st.button("Otimizar PDF e Preparar Download", key="optimize_pdf_button", disabled=st.session_state.get('processing_optimize', False)):
                ...
                with st.spinner("A otimizar o PDF... Por favor, aguarde."):
                    try:
                        ...
                        if selected_profile_name == "Leve (Rápida, Sem Perda de Qualidade Visual)": save_options_optimize.update({"garbage": 2, "deflate": True})
                        elif selected_profile_name == "Recomendada (Equilíbrio entre Tamanho e Qualidade)": save_options_optimize.update({"garbage": 4, "deflate": True, "deflate_images": True, "deflate_fonts": True})
                        elif selected_profile_name == "Máxima Tentativa (Pode Afetar Qualidade de Imagens)": save_options_optimize.update({"garbage": 4, "deflate": True, "deflate_images": True, "deflate_fonts": True})
                        ...
                        if original_size_bytes > 0 and optimized_size_bytes < original_size_bytes:
                            ...
                            st.info("O tamanho do ficheiro não foi reduzido (pode até ter aumentado ligeiramente).")
